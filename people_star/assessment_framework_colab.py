...
class assess_framework():
    ...
    def assess(self):
        ...
        if "SSD" in self.study:
            ...
            distance_v['SSD'] = self.assess_v(params[0], params[1])
            distance_repul_h['SSD'] = self.assess_repul_h(params[2], params[3])
            distance_repul_m['SSD'] = self.assess_repul_m(params[4], params[5])
            distance_p['SSD'] = self.assess_p(params[6], params[7])

        if "SAD" in self.study:
            params = self.__get_best_params('SAD')
            distance_v['SAD'] = self.assess_v(params[0], params[1])
            distance_repul_h['SAD'] = self.assess_repul_h(params[2], params[3])
            distance_repul_m['SAD'] = self.assess_repul_m(params[4], params[5])
            distance_p['SAD'] = self.assess_p(params[6], params[7])

        if "KL" in self.study:
            params = self.__get_best_params('KL')
            distance_v['KL'] = self.assess_v(params[0], params[1])
            distance_repul_h['KL'] = self.assess_repul_h(params[2], params[3])
            distance_repul_m['KL'] = self.assess_repul_m(params[4], params[5])
            distance_p['KL'] = self.assess_p(params[6], params[7])

        if "ZNCC" in self.study:
            params = self.__get_best_params('ZNCC')
            distance_v['ZNCC'] = self.assess_v(params[0], params[1])
            distance_repul_h['ZNCC'] = self.assess_repul_h(params[2], params[3])
            distance_repul_m['ZNCC'] = self.assess_repul_m(params[4], params[5])
            distance_p['ZNCC'] = self.assess_p(params[6], params[7])
